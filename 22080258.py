...
fig, axs = plt.subplots(2, 2, figsize=(15, 15), gridspec_kw={
...
axs[0, 0].set_title(r'$\bf{Age\ Distribution}$',
                    color='red')  # Bold and red headline
...
sns.barplot(x='Sleep Duration', y='Occupation',
            data=data, palette='viridis', ax=axs[0, 1])
# Bold and red headline
axs[0, 1].set_title(
    r'$\bf{Average\ Sleep\ Duration\ by\ Occupation}$', color='red')
...
data['BMI Category'].value_counts().plot.pie(
    autopct='%1.1f%%', colors=sns.color_palette('pastel'), ax=axs[1, 0])
axs[1, 0].set_title(r'$\bf{BMI\ Category\ Distribution}$',
                    color='red')  # Bold and red headline
axs[1, 0].set_ylabel('')
axs[1, 0].legend(data['BMI Category'].value_counts().index,
...
sns.violinplot(x='Sleep Disorder', y='Heart Rate',
               data=data, palette='muted', ax=axs[1, 1])
# Bold and red headline
axs[1, 1].set_title(
    r'$\bf{Heart\ Rate\ Distribution\ by\ Sleep\ Disorder}$', color='red')
...
fig.suptitle(r'$\bf{Sleep\ Health\ and\ Lifestyle}$',
             y=1.0, fontsize=16)  # Bold title
fig.text(0.5, 0.02, summary_text, ha='center',
